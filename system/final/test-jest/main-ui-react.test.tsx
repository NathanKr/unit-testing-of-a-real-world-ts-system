...
import { ButtonsText, getEnumKeyValues } from "../test/test-utils";
...
test("button enqueueGetPosts invoked --> getPosts and 100 in output", async () => {
  ...
  await waitFor(
    () => {
      expect(outputElem.textContent).toContain("100");
      expect(outputElem.textContent).toContain(ActionType.getPosts);
    },
  ...
  test("click on add -> 3 and add appears in the output", async () => {
    ...
    await waitFor(
      () => {
        ...
        expect(outputElem.textContent).toContain(ActionType.add)
      },
      { timeout: SCHEDULER_INTERVAL_SEC * 1000 * 2 }
    );
  });
...
test("failure status is add --> failure to appear in the ui", async () => {
    const spyOnAdd = jest.spyOn(functions, ActionType.add);
